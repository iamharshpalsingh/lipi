...
const DOCS: &[(&str, &str, &str)] = &[
    ...
    ("field", "field value, placeholder: \"...\" with value", "A text box. The block runs with the new text on every change. `lines: 4` makes it that many lines tall."),
    ("checkbox", "checkbox checked, label with checked", "A checkbox. The block runs with true or false when it changes."),
    ("select", "select value, options: [...] with chosen", "A dropdown. An option is a value or {value, label}; the block runs with the chosen value."),
    ("upload", "upload label, accept: \"image/*\" with files", "Choose files. The block runs with an Array of {name, type, size, dataUrl, text}."),
    ...
    ("js.global", "js.global", "The JavaScript global object (`window` in a browser). JavaScript builds only."),
    ("js.import", "js.import(module) -> Task", "Loads a JavaScript module: a URL, a file served next to index.html, or a Node package. Use with await."),
    ("js.new", "js.new(constructor, ...args)", "Creates a JavaScript object, like `new` in JavaScript."),
    ("js.value", "js.value(object)", "Copies plain JavaScript data (objects and arrays) into LiPi values."),
    ("js.typeOf", "js.typeOf(value) -> String", "JavaScript's typeof for a value."),
