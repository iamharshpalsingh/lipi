...
pub fn parse(it: &Interpreter, text: &str, span: Span) -> Result<Value, Flow> {
    match serde_json::from_str::<serde_json::Value>(text) {
        ...
        Err(e) => Err(it.error(
            ...
            span,
            Some("JSON looks like {\"name\": \"Dezy\", \"age\": 25}. Keys and text need double quotes. Tip: write JSON in 'single quotes'.".into()),
        )),
    }
}
...
pub fn to_json(v: &Value, depth: usize) -> Result<serde_json::Value, String> {
    ...
    Ok(match v {
        ...
        Value::Num(n) => serde_json::Number::from_f64(*n).map(serde_json::Value::Number).ok_or_else(|| format!("{n} can't be written as JSON"))?,
